...
it('matches the customer card totals, currency symbols, details and return path', async () => {
  customerMocks.getById.mockResolvedValue({
    id: 'customer-1',
    name: 'Ірина',
    phone: '+380501112233',
  ...
  expect(
    await screen.findByRole('link', { name: 'До клієнтів' }),
  ).toHaveAttribute('href', '/app/garage/customers')
  expect(screen.getByText('5 250 $')).toBeVisible()
  expect(screen.getByText('10 500 ₴')).toBeVisible()
  const details = screen.getByText('Деталі').closest('section')
  expect(details).not.toHaveTextContent('Телефон')
})

it('uses stable varied avatar colours for different customers', async () => {
  customerMocks.list.mockResolvedValue({
    items: [
      {
        id: 'a',
  ...
  const anna = await screen.findByText('А')
  const bohdan = screen.getByText('Б')
  expect(anna.className).not.toBe(bohdan.className)
})
